...
def undo_command(event = None):                                       #Undo Option
    try:
        text.edit_undo()
        lnupdater()
    except TclError:
...
def redo_command(event = None):                                       #Redo Option
    try:
        text.edit_redo()
        lnupdater()
    except TclError:
        pass
def save_command(event = None):                                       #save option
...
def highlight_line(interval = 100):                                                #Highlight Text
...
def undo_highlight():                                                              #Undo Highlight
...
helpmenu.add_command(label='About')                                              #options in HELP Menu
